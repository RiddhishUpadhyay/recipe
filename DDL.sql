...
name varchar(20) not null,
cate_id1 decimal(9,0) references rp.cate(cate_id) on delete restrict on
update cascade
);
create table rp.tod(
tod_id decimal(9,0) primary key,
name varchar(20) not null,
cate_id2 decimal(9,0) references rp.cate(cate_id)on delete restrict on update
cascade
);
create table rp.season(
season_id decimal(9,0) primary key,
name varchar(20) not null,
cate_id3 decimal(9,0) references rp.cate(cate_id)on delete restrict on update
cascade
);
...
recipe_id decimal(9,0) references rp.recipe(recipe_id) on delete cascade on
update cascade,
dishtype_id decimal(9,0) references rp.dishtype(dishtype_id) on delete set
null on update cascade,
tod_id decimal(9,0) references rp.tod(tod_id) on delete set null on update
cascade,
season_id decimal(9,0) references rp.season(season_id) on delete set null on
update cascade
);
...
recipe_id decimal(9,0) references rp.recipe(recipe_id)on delete set null on
update cascade,
ingre_id decimal(4,0) references rp.ingredient(ingre_id)on delete set null on
update cascade
);
